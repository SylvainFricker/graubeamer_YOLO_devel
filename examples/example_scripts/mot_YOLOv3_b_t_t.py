...
white_plane = np.zeros([700,900,3],dtype=np.uint8)
...
while True:
    ...
    transfomred_bboxes = Transform.get_bboxes_transformed(bboxes,rl,ll,rt,lt)
    print("transbb:\n",transfomred_bboxes)
    transbb_drawn = model.draw_bboxes(white_plane.copy(),transfomred_bboxes,confidences,class_ids)
    cv.imshow("transfomred_bboxes",transbb_drawn)
    ...
    transformed_bboxes_and_centroids = Transform.Transform_and_draw_centroids(transbb_drawn,tracks,rl,ll,rt,lt)
    cv.imshow("transformed_bboxes_and_centroids",transformed_bboxes_and_centroids)

    tt , transformed_bboxes= Transform.old_get_all4points_bboxes_transformed_and_draw_them(white_plane.copy(),bboxes,rl,ll,rt,lt)
    cv.imshow("tts",tt)
