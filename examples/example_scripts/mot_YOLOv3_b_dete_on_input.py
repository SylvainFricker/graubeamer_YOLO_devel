...
while True:
    ...
    print(confidences)
    print(class_ids)
    #print("bboxes transfomred:\n",bboxes)
    updated_image = model.draw_bboxes(image.copy(), bboxes, confidences, class_ids)
    tracks,updated_image = tracker.update2(bboxes, confidences, class_ids,updated_image)
    ...
    transformed_bboxes = Transform.get_all4points_bboxes_transformed(white_plane.copy(),bboxes,rl,ll,rt,lt)
    show = model.draw_bboxes(white_plane.copy(),transformed_bboxes,confidences,class_ids)
    cv.imshow("ii",show)
    transformed_bboxes_and_centroids = Transform.Transform_and_draw_centroids(show,tracks,rl,ll,rt,lt)
    cv.imshow("transformed_bboxes",transformed_bboxes_and_centroids)
