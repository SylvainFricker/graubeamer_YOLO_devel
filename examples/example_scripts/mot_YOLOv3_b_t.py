...
white_plane = np.zeros([800,800,3],dtype=np.uint8)
...
while True:
    ...
    #wir wollen aber nur Mittelpunkte transformieren
    '''
    white_plane = np.zeros([1080,1920,3],dtype=np.uint8)
    white_plane.fill(255)
    white_plane = draw_tracks(white_plane, tracks)
    white_plane = Transform.PerspectiveTransformation(white_plane,lt,rt,ll,rl)
    cv.imshow("white",white_plane)
    '''
    
    white_plane = Transform.Transform_and_draw_centroids(white_plane, tracks,rl,ll,rt,lt)
    cv.imshow("white",white_plane)
