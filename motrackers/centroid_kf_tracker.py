...
def assign_tracks2detection_centroid_distances(bbox_tracks, bbox_detections, distance_threshold=1000.):
    """
    ...
    #print("estimated track centroids:\n",estimated_track_centroids)
    detection_centroids = get_centroid(bbox_detections)
    #print("detection centroids:\n",detection_centroids)
    centroid_distances = distance.cdist(estimated_track_centroids, detection_centroids)
    centroid_distances[centroid_distances<15] = 0

    assigned_tracks, assigned_detections = linear_sum_assignment(centroid_distances)
    #print("assignet tracks\n",assigned_tracks)
    #print("assignet detections\n",assigned_detections)
    cost_matrix = centroid_distances[assigned_tracks, assigned_detections].sum()
    #print("cost_matrix\n",cost_matrix)
...
class CentroidKF_Tracker(Tracker):
    ...
    def __init__(
            self,
            max_lost=20,
            centroid_distance_threshold=1000.,
    ...
    def update2(self, bboxes, detection_scores, class_ids,img):
        self.frame_count += 1
        bbox_detections = np.array(bboxes, dtype='int')
        #print("bbox_detections:\n",bbox_detections)
        #print("bboxes:\n",bboxes)
        ...
        img = Transform.draw_bboxes(img,bbox_tracks)
