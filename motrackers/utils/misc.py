...
def get_corners_of_ROI_as_int():
    ...
    #ilanz 1920-1080
    #rl,ll,rt,lt = np.int32([749,696]) , np.int32([1306,721]) , np.int32([389,973]) , np.int32([1561,1014])

    #ilanz 1400-1000
    rl,ll,rt,lt = np.int32([546,644]) , np.int32([953,667]) , np.int32([283,900]) , np.int32([1139,938])

    #davos 1400-1000
    #rl,ll,rt,lt = np.int32([350,700]) , np.int32([920,740]) , np.int32([100,1000]) , np.int32([1100,1000])

    #rl,ll,rt,lt = np.int32([500,600]) , np.int32([920,640]) , np.int32([240,1000]) , np.int32([1100,1000])

    corners = np.int32([rl,ll,rt,lt])
    return rl,ll,rt,lt,corners


def check_if_detection_in_ROI(x,y):
    p1,p2,p3,p4,corners = get_corners_of_ROI_as_int()
    poly = [p1,p2,p3,p4]
    bbPath = mplPath.Path(np.array([ poly[0] , poly[1] , poly[3] , poly[2] ]))
    return bbPath.contains_point((x,y))


def check_if_detection_in_ROI_v2(x,y,corners):
    bbPath = mplPath.Path(np.array([ corners[0] , corners[1] , corners[3] , corners[2] ]))
    return bbPath.contains_point((x,y))
